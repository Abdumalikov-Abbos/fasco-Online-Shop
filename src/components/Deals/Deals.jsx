...
export default function Discounts({ duration = 1000 * 60 * 60 * 24 * 5 }) {
  ...
  return (
    <div
      id="deals"
      className="flex flex-col md:flex-col lg:flex-col xl:flex-row justify-between items-center px-4 md:px-8 lg:px-16 xl:px-24 py-10 bg-gray-50 gap-10"
    >
      {/* Left Side */}
...
function TimeBox({ value, label }) {
  return (
    <div className="shadow-midNight shadow-lg rounded-lg p-4 flex flex-col items-center justify-center w-[80px] h-[80px] md:w-[90px] md:h-[90px] bg-white">
      ...
      <span className="text-sm md:text-lg font-medium text-black mt-1 md:mt-2">
        {label}
      </span>
    </div>
  );
