...
export default function Product() {
  ...
  const calculateValue = (remainingItems, initialStock) => {
    ...
    const percentage = Math.round(
      ((initialStock - remainingItems) / initialStock) * 100,
    );
    return Math.max(0, Math.min(percentage, 100));
  };

  const resultValue =
    product.items && 500 ? calculateValue(product.items, 500) : 0;
  ...
  return (
    <>
      <main className="pt-[52px] max-w-fasco-container mx-auto">
        ...
        <section className="flex pt-14">
          <aside className="flex items-center pl-8">
            <div className="grid items-center gap-3">
              <img
                src={product1}
                alt=""
                className="w-[58px] h-[77px] hover:scale-105 duration-500"
                onClick={() => handleClickPhoto(2)}
              />
              <img
                src={product.img}
                alt={product.title}
                className={`w-[58px] h-[77px] hover:scale-105 duration-500`}
                onClick={() => handleClickPhoto(1)}
              />
              <img
                src={product2}
                alt=""
                className="w-[58px] h-[77px] hover:scale-105 duration-500"
                onClick={() => handleClickPhoto(3)}
              />
              <img
                src={product3}
                alt=""
                className="w-[58px] h-[77px] hover:scale-105 duration-500"
                onClick={() => handleClickPhoto(4)}
              />
              <img
                src={product4}
                alt=""
                className="w-[58px] h-[77px] hover:scale-105 duration-500"
                onClick={() => handleClickPhoto(5)}
              />
              <img
                src={product5}
                alt=""
                className="w-[58px] h-[77px] hover:scale-105 duration-500"
                onClick={() => handleClickPhoto(6)}
              />
            </div>
            <div className="pl-4">
              <img
                src={product.img}
                alt={product.title}
                className={`h-[520px] hover:scale-105 duration-500`}
                onClick={() => handleClickPhoto(1)}
              />
            </div>
            {photo === 1 && (
          ...
          <div className="pl-14">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                className=" w-[45px] h-[45px] rounded-full border border-Gray"
                onClick={handleClickStar}
              >
                <span className={`text-2xl ${clickStar ? "text-yellow" : ""}`}>
                  ★
                </span>
              </button>
            </div>
            ...
            <div className="flex items-center pt-4 pb-4">
              <img src={eye} alt="" className="pr-4" />
              <span className="text-lg text-Gray">
                {randomViewers} people are viewing this right now
              </span>
            </div>
            <div className="flex items-center justify-between text-xl font-medium pt-2 pb-2 pl-2 pr-2 rounded-lg bg-pastelPink text-coralPink border-softPastelPink border-2">
              ...
              <div className="flex items-center">
                <span className="pr-2">
                  {String(Math.floor(time / (1000 * 60 * 60 * 24))).padStart(
                    2,
                    "0",
                  )}
                  :{" "}
                ...
                <span className="pr-2">
                  {String(Math.floor((time % (1000 * 60)) / 1000)).padStart(
                    2,
                    "0",
                  )}
                </span>
            ...
            <h4 className="pt-2">
              Only <span className="font-medium">{product.items}</span> item(s)
              left in stock!
            </h4>
            ...
            <h4 className="text-lg text-black font-medium pb-2 pt-2">
              Quantity
            </h4>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-6">
                <button className="text-2xl" onClick={() => updateCount(-1)}>
                  -
                </button>
                <span className="text-lg text-black font-medium">{count}</span>
                <button className="text-2xl" onClick={() => updateCount(1)}>
                  +
                </button>
              </div>
              <button
                className="hover:bg-black hover:text-white duration-500 border-[2px] rounded-lg pt-2 pb-2 pr-[80px] pl-[80px] border-black text-lg font-medium text-black"
            ...
            <div className="flex items-center gap-6 pt-4">
              <div className="flex items-center gap-2 text-lg font-medium text-black hover:text-midNight duration-500 cursor-pointer">
                <img src={Compare} alt="" />
                <span>Compare</span>
              </div>
            ...
            <div className="flex items-center gap-2 text-black pt-4">
              ...
              <div className="flex items-center text-lg gap-2">
                <h5 className="font-medium">Estimated Delivery:</h5>
            ...
            <div className="flex items-center gap-2 text-black pt-4">
              ...
              <div className="flex items-center text-lg gap-2">
                <h5 className="font-medium">Free Shipping & Returns:</h5>
            ...
            <div className="grid justify-center items-center">
              ...
              <h5 className="flex justify-center text-lg text-black pt-4 font-medium">
            ...
            <div className="fixed inset-0 bg-midNight bg-opacity-70 z-50 flex items-center justify-center">
              <div className="fixed top-0 right-0 bg-white h-full w-2/4 pl-12 pr-12 pt-1  overflow-y-auto">
                <div className="flex justify-between items-center">
                  <h3 className="text-[42px] leading-[32px] text-black font-[400] pb-4 mt-[45px]">
                    Shopping Cart
                  </h3>
                  <button
                ...
                <div className="flex items-center text-[26px] gap-2 mt-[26px]">
                  <span>Buy</span>
                  <h4 className="text-black font-[700] text-[26px] leading-[32px]">
                    $122.35
                  </h4>
                  More and Get{" "}
                  <h4 className="text-black font-[700] text-[26px] leading-[32px]">
                ...
                <div className="flex pt-8 gap-6 mt-[60px] ">
                  <img
                    src={product.img}
                    alt=""
                    className="w-[200px] hover:scale-105 duration-500"
                  />
                  <div>
                    <h4 className="text-black font-medium text-2xl pt-4">
                      {product.title}
                    </h4>
