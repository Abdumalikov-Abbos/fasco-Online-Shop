        ...
        <div className="flex gap-4 pr-8 relative">
          ...
          <div className="relative">
            ...
            {cartProducts.length > 0 && (
              <Link to={'/cart'} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">
                {cartProducts.length}
              </Link>
            )}
          </div>
      ...
      <section>
        <div className='grid justify-center pt-20'>
          ...
          <div className='flex items-center justify-center text-black pt-4 gap-6'>
            <Link to="/" className='link'>Home</Link>
            <img src={Arrow} alt="" className='flex justify-center -rotate-90 w-3' />
        ...
        <div className='pt-14 pr-8 pl-8'>
          ...
          {cartProducts.length > 0 ? (
            cartProducts.map(item => (
              <div key={item.id} className='mt-[35px] flex justify-between items-center'>
                ...
                <span className='text-black text-xl font-medium'>${item.price}.00</span>

                <div className="flex items-center gap-4">
                  <button className="text-2xl text-black" onClick={handleModalDecrement}>-</button>
                  <span className="text-2xl">{formatNumber(count)}</span>
                  <button className="text-2xl text-black" onClick={handleModalIncrement}>+</button>
                </div>
            ...
            <p className="text-center text-xl text-gray-600 pt-10">Cart is empty!</p>
          )}
          ...
          <div className='grid justify-evenly'>
            ...
            <div className='flex items-center justify-between pt-14 text-2xl text-black font-medium'>
              <span>Subtotal</span>
              <span>
                ${cartProducts.reduce((total, item) => total + item.price * count, 0) + (check ? 10 : 0)}.00
              </span>
            </div>
